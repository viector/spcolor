...
from utils.util import uncenter_l, vgg_preprocess,batch_lab2rgb_transpose_mc,save_frames
import numpy as np
...
def prepare_l_tosave(img):  # input 0~1     b c h w / c h w
    if img.dim()==4:
        img = img[0]
    img = img.cpu().numpy()
    img = (np.clip(img.transpose(1, 2, 0), 0, 1) * 255).astype("uint8")
    return img
...
class WarpNet(nn.Module):
    ...
    def __init__(self, batch_size):
        ...
        self.target_point_h = 50
        self.target_point_w = 23
        self.point_pos=-1
        self.tp_ref = None
    ...
    def non_local(self,A_features,B_features,B_abs,batch_size=1,temperature=0.01):
        ...
        if self.mode_targetp and self.point_pos>=0:
            f_tp = f  / 10
            f_div_tp = F.softmax(f_tp, dim=-1)  # 1*lenA*lenB
            target_warp_img = f_div_tp[:,[self.point_pos],:] # 1 1 lenB
            #target_warp_img = target_warp_img/2 +0.3
            target_warp_img = (target_warp_img - torch.min(target_warp_img)) / torch.max(target_warp_img)
            self.tp_ref = target_warp_img
            # twi = target_warp_img.view(1,1,64,64)
            # twi = twi/2 + 0.5
            # print("checkpoint 0")
        ...
        B_abs = B_abs.permute(0, 2, 1)  # 1*lenB*3

        # multiply the corr map with color
        y = torch.matmul(f_div_C, B_abs)  # 1*lenA*3
        y = y.permute(0, 2, 1).contiguous() # 1*3*lenA
        sc_ref_w = y[:,2:3,:]
        y = y[:,0:2,:]
        similarity_map = similarity_map * sc_ref_w
        return y , similarity_map  # 1*3*lenA  # 1*lenA
    ...
    def forward(
        ...
        cluster_value_ref = F.interpolate(cluster_value_ref.unsqueeze(1),feature_size,mode = "nearest")
        ...
        B_abs = torch.cat([B_ab,cluster_value_ref],dim=1)
        for i_batch in range(batch_size):
            ...
            if self.mode_targetp: # find target point correpondence
                ...
                tpm = target_point_map[:,[0],:,:]
                tpm = prepare_l_tosave(tpm)
                save_frames(tpm,"/dataset/temp",image_name="tpm.png")
                print("saved tpm")
