...
image_size_h, image_size_w = 299 , 299
...
class PartialInceptionNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        assert x.shape[1:] == (3, image_size_h, image_size_w), "Expected input shape to be: (N,3,299,299)" +\
...
def get_activations(images, batch_size):
    ...
    """
    assert images.shape[1:] == (3, image_size_h, image_size_w), "Expected input shape to be: (N,3,299,299)" +\
...
def preprocess_image(im):
    ...
    im = np.rollaxis(im, axis=2)    # h w 3 to 3 h w 
    im = torch.from_numpy(im)
    assert im.max() <= 1.0
    assert im.min() >= 0.0
    assert im.dtype == torch.float32
    assert im.shape == (3, image_size_h, image_size_w) , "shape error:{}".format(im.shape)
    ...
    if use_multiprocessing:
        with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
            ...
            for im in images:
                ...
                jobs.append(job)
            final_images = torch.zeros(images.shape[0], 3, image_size_h, image_size_w)
            for idx, job in enumerate(jobs):
                im = job.get()
                final_images[idx] = im#job.get()
    else:
        final_images = torch.stack([preprocess_image(im) for im in images], dim=0)
    assert final_images.shape == (images.shape[0], 3, image_size_h, image_size_w)
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option("--p1", "--path1", dest="path1", default = '/dataset/ImageNet_val/val_10000/croped_imgs',
                      help="Path to directory containing the real images")
    parser.add_option("--p2", "--path2", dest="path2", default = '/dataset/checkpoints/deepexemplar/val_10000/2',
    ...
    parser.add_option("-b", "--batch-size", dest="batch_size",default=16,
                      ...
                      type=int)
    parser.add_option("--gray_mode", default=False)          
