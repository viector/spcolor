...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        print("may here in clusterlookup:",dim,n_classes,cfg.extra_clusters)
    ...
    def prepare_pre_cluster(self,label,cluster,n):
        class_dict = {}
        cluster_new = torch.zeros([n,90])
        _n = 0
        for i in range(27):
            if label[i] not in class_dict:   # 16 in class_dict ?
                class_dict[label[i]]=i       # class_dict: {16: 0}
                cluster_new[_n]=cluster[i]
                _n+=1
        print("done pre cluster, _n :",_n)
        return cluster_new

    def redefine_cluster(self,n_clusters=27,mode=["Hierarchical","post"]):
        self.cluster_probe.clusters.requires_grad = False
        n=n_clusters
        #self.cluster_probe = ClusterLookup(90, n)
        if mode[0] == "Hierarchical":
            cluster_model = AgglomerativeClustering(n_clusters=n, affinity='euclidean', linkage='ward')  # 'cityblock'  'euclidean' 'average' 'ward'
            cluster_model.fit(self.cluster_probe.clusters)
            cluster_label = cluster_model.labels_
            cluster_new = self.prepare_pre_cluster(cluster_label,self.cluster_probe.clusters,n)
        else:
            np.random.seed(122)
            cluster_new,cluster_label = sf_kmeans(self.cluster_probe.clusters,n_clusters=n)
            cluster_label = cluster_label.numpy()

        if n_clusters < 27:
            print("Silhouette Coefficient: %0.3f"
                    % (silhouette_score(self.cluster_probe.clusters, cluster_label, metric='sqeuclidean')))
        #------pre cluster------#
        if mode[1]== "pre":
            self.cluster_probe.clusters = torch.nn.parameter.Parameter(data=cluster_new, requires_grad=False)
            self.cluster_probe.n_classes=n
            return None
        else:
