...
def sf_kmeans(matrix, n_clusters):
    # kmeans
    print("shape bf kmeans:",matrix.shape)
    cluster_ids_x, cluster_centers = kmeans(
    X=matrix, num_clusters=n_clusters, distance='euclidean', device=matrix.device
    )
    #print("shape af kmeans:",cluster_centers.shape,cluster_ids_x.shape)
...
class ClusterLookup(nn.Module):
    ...
    def forward(self, x, alpha, log_probs=False,soft_probs=False):
        ...
        if log_probs:
            ...
            return nn.functional.softmax(inner_products * alpha, dim=1)
        else:
